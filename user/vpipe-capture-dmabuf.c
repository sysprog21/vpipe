...
int main(int argc, char **argv)
{
    ...
    const char *csv = argc > 3 ? argv[3] : "bench/dmabuf.csv";
    ...
    fp = vpipe_open_csv(
        csv,
        "frame,vivid_sequence,vpipe_sequence,vivid_buf_index,dmabuf_fd,vivid_"
        "dequeue_ns,vpipe_dequeue_ns,vpipe_bytesused");
    if (!fp) {
        perror("csv");
        goto cleanup;
    }
    ...
    for (i = 0; i < (unsigned int) frames_ul; i++) {
        ...
        if (set_ctrl(vpipe_fd, VPIPE_CID_SRC_SEQUENCE, vivid_dq.sequence) < 0 ||
            set_ctrl(vpipe_fd, VPIPE_CID_ALGO, VPIPE_ALGO_NONE) < 0) {
        ...
        fprintf(fp, "%u,%u,%u,%u,%d,%" PRIu64 ",%" PRIu64 ",%u\n", i,
                vivid_dq.sequence, vpipe_dq.sequence, vivid_dq.index,
                vivid_bufs[vivid_dq.index].dmabuf_fd, vivid_ns, vpipe_ns,
                vpipe_dq.bytesused);
